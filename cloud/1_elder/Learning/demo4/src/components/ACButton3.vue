<template>
  <div>
    ...
    <button @click="toggleAutomation"
      :style="{ fontSize: '35px', backgroundColor: buttonBackgroundColor, marginLeft: '0%' }">{{
        automationButtonText }}</button>
  </div>
</template>
...
export default {
  ...
  computed: {
    automationButtonText() {
      return this.isAutomated ? '关闭自动增湿' : '打开自动增湿'; // 根据自动化状态返回相应文本内容
    },
  ...
  methods: {
    ...
    startTask() {
      ...
      const apiUrl = `http://8.130.72.214:3000/autotask/set?device=water&state=1&threshold=${thresholdValue}`;
    ...
    stopTask(){
      const thresholdValue = this.threshold; // 假设这是你的变量值
      const apiUrl = `http://8.130.72.214:3000/autotask/set?device=water&state=0&threshold=${thresholdValue}`;
      fetch(apiUrl) // 替换成你的 API 地址
        .then(response => {
          if (!response.ok) {
            throw new Error('Network response was not ok');
