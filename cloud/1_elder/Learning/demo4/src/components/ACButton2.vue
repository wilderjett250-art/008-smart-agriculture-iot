...
export default {
  ...
  computed: {
    automationButtonText() {
      return this.isAutomated ? '关闭自动补光' : '打开自动补光'; // 根据自动化状态返回相应文本内容
    },
  ...
  methods: {
    ...
    startTask() {
      ...
      const apiUrl = `http://8.130.72.214:3000/autotask/set?device=light&state=1&threshold=${thresholdValue}`;
    ...
    stopTask(){
      const thresholdValue = this.threshold; // 假设这是你的变量值
      const apiUrl = `http://8.130.72.214:3000/autotask/set?device=light&state=0&threshold=${thresholdValue}`;
      fetch(apiUrl) // 替换成你的 API 地址
        .then(response => {
          if (!response.ok) {
            throw new Error('Network response was not ok');
